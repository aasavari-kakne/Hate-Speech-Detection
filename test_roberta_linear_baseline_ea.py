...
def train(num_epochs, model):
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1_score > best_val_f1:
            ...
            np.save(os.path.join(DATA_DIR, 'label_history_ea_weighted.npy'), list(zip(val_actual_labels, 
                                                            val_predicted_labels, prob_neg_label, prob_pos_label, val_tweet_ids)))
            save(model, epoch, optimizer, np.mean(val_loss_arr), model_prefix='roberta_linear_baseline_model_ea_weighted')
        ...
        np.save(os.path.join(DATA_DIR, 'history_ea_weighted.npy'), history)
